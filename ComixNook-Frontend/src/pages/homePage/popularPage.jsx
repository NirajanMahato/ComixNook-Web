...
const PopularPage = () =>{
    ...
    return(
        <>
            <div className={"md:h-96 h-auto md:p-10 p-5 mb-24"}>
                <h1 className={"md:text-4xl text-3xl gilroy-bold"}>Popular this week.</h1>
                <div className={"pt-6 md:flex justify-between"}>
                    {popularItems?.map((i) => (
                        <div key={i?.itemId}
                             className={"md:w-[23rem] md:mb-0 mb-8 rounded-xl cursor-pointer shadow-xl hover:shadow-2xl hover:bg-gray-100 transform-gpu scale-100 hover:scale-105 transition-transform duration-500"}>
